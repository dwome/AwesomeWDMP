...
public class ErrorAnalysis {
	...
	public void printFalsePositives(Processable<Correspondence<Club, Attribute>> correspondences, MatchingGoldStandard gs) {
		
		// go through the correspondences and check if they are incorrect
		for(Correspondence<Club, Attribute> c : correspondences.get()) {
			
			// is the match incorrect?
			if(gs.containsNegative(c.getFirstRecord(), c.getSecondRecord())) {
				
				// if yes, print the records to the console
				Club m1 = c.getFirstRecord();
				Club m2 = c.getSecondRecord();
	...
	public void printFalseNegatives(DataSet<Club, Attribute> ds1, DataSet<Club, Attribute> ds2, Processable<Correspondence<Club, Attribute>> correspondences, MatchingGoldStandard gs) {
		...
		for(Pair<String, String> p : allPairs) {
			...
			Club m1 = ds1.getRecord(p.getFirst());
			if(m1==null) {
				m1 = ds2.getRecord(p.getFirst());
			}
			
			// get the second record
			Club m2 = ds1.getRecord(p.getSecond());
			if(m2==null) {
