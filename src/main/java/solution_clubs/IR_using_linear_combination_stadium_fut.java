...
public class IR_using_linear_combination_stadium_fut {
	public static void main(String[] args) throws Exception {
		...
		Processable<Correspondence<Club, Attribute>> correspondences = engine.runIdentityResolution(dataFut17, dataStadium, null, matchingRule,blocker2);

		// write the correspondences to the output file
		new CSVCorrespondenceFormatter().writeCSV(new File("data/output/fut17_2_stadium_correspondences.csv"),
				correspondences);
		
		// load the gold standard (test set)
				MatchingGoldStandard gsTest = new MatchingGoldStandard();
				gsTest.loadFromCSVFile(new File(
						"data/goldstandard/gs_club_stadium_2_fut17.csv"));
				...
				new ErrorAnalysis().printFalseNegatives(dataFut17, dataStadium, correspondences, gsTest);
				
				// print the evaluation result
				System.out.println("Fut17 <-> Stadium");
				System.out
