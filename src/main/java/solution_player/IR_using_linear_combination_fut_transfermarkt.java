...
import blocker.PlayerBlockingFunctionName;
...
public class IR_using_linear_combination_fut_transfermarkt {
	public static void main(String[] args) throws Exception {
		...
		// add comparators
		matchingRule.addComparator(new PlayerNameComparatorJaccard(), 0.9);
		...
		StandardRecordBlocker<Player, Attribute> blocker2 = new StandardRecordBlocker<Player, Attribute>(
				new PlayerBlockingFunctionName());
		...
		Processable<Correspondence<Player, Attribute>> correspondences = engine.runIdentityResolution(
				dataFut17, dataTrans, null, matchingRule,
				blocker2);
		...
		// load the gold standard (test set)
				...
				gsTest.loadFromCSVFile(new File(
						"data/goldstandard/gs_fut17_2_trans.csv"));
