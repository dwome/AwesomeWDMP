...
public class IR_using_linear_combination_fifa_transfermarkt {
	public static void main(String[] args) throws Exception {
		...
		for (double i = 0.1; i < 1.5; i += 0.1) {
			for (double j = 0.1; j < 1.5; j += 0.1) {
				for (double k = 0.1; k < 1.5; k += 0.1) {
					for (double l = 0.1; l < 1.5; l += 0.1) {
						System.out.println("Name: " + i);
						System.out.println("Height: " + j);
						System.out.println("Birthdate: " + k);
						System.out.println("Weight: " + l);
						double tempscore = parameterTuner(i, j, k, l);
						if (tempscore >= score) {
							score = tempscore;
							bestN = i;
							bestH = j;
	...
	public static double parameterTuner(double name, double weight, double height, double bd) throws Exception {
		// loading data
